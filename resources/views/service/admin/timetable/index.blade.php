            ...
            <div class="col-md">
                <div class="card">
                    <div class="card-header">{{ __('Расписание ') }}
                        @if(cache('timetable'))
                            <a class="btn btn-danger p-1 position-absolute" style="top: 3px; right: 10px" href="{{route('admin.timetable.toggle')}}" role="button">Выключить отображение</a>
                        @else
                            <a class="btn btn-primary p-1 position-absolute" style="top: 3px; right: 10px" href="{{route('admin.timetable.toggle')}}" role="button">Включить отображение</a>
                        @endif
                            ...
                            @forelse($days as $day)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a class="btn btn-outline-danger" href="{{route('admin.timetable.delete', ['id' => $day->id])}}" onclick='return confirm("Вы уверены, что хотите удалить день со всеми входящими в него событиями?");' role="button">
                                ...
                                @forelse($day->timelines as $timeline)
                                    <tr>
                                        <td class="text-muted">{{{$timeline->from->format('H:i')}}} - {{{$timeline->to->format('H:i')}}}</td>
                                        <td class="text-muted" colspan="6">{!!nl2br(htmlspecialchars($timeline->description))!!}</td>
                                    </tr>
                                @empty
                                    <tr class="text-muted">
